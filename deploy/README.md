...
Salt **`notam/timers.sls`** installs `notam-sync.timer` (every 3 min) and `notam-reconcile.timer` (02:17 daily). Units use **`EnvironmentFile=../.env`** (absolute path on disk). Logs go to **journald**:
